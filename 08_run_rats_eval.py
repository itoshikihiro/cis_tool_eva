...
def run_rats_and_extract_line_numbers(file_path):
    # Run RATS on the given file path
...
def process_csv_file(input_csv_path, output_csv_path):
    ...
    with open(input_csv_path, 'r', newline='', encoding='utf-8') as file:
        ...
        for row in reader:
            file_path = row[0]
            line_numbers = run_rats_and_extract_line_numbers(file_path)
...
output_csv_file_path = '/workspaces/final_project/dataset/rats_output.csv'
